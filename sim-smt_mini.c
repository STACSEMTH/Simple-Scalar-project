...
void sim_reg_stats(struct stat_sdb_t *sdb) {
  ...
  stat_reg_counter(sdb, "fetch_ifq_full",  "fetch stalls: IFQ full",  &fetch_ifq_full,  0, NULL);
  stat_reg_counter(sdb, "rename_rob_full", "rename stalls: ROB full", &rename_rob_full, 0, NULL);
  stat_reg_counter(sdb, "rename_lsq_full", "rename stalls: LSQ full", &rename_lsq_full, 0, NULL);
...
void sim_main(void) {
  while (1) {
    ...
    for (int t=0;t<num_hw_threads;++t) {
      alive |= tctx[t].active; 
      rob_empty &= (tctx[t].rob_head == tctx[t].rob_tail);
    }
    if (!alive && rob_empty) break;
...
static void fetch_stage(void) {
  ...
  for (int tries = 0;tries<num_hw_threads && fetched < sim_outorder_width;++tries) {
    ...
    if ((rand() & 0xFF) == 0){ /* mispred */
        ...
        for (int t = 0; t < num_hw_threads; ++t) {
          for (int r = 0; r < MD_TOTAL_REGS; ++r) tctx[t].rename_map[r] = r;
        }
        rob_head_global = rob_tail_global; /* flush ROB */
        ifq_head = ifq_tail; /* flush IFQ */
        lsq_head = lsq_tail; /* flush LSQ */
        for (int tt=0; tt < num_hw_threads; ++tt){
          tctx[tt].rob_head = tctx[tt].rob_tail = rob_tail_global;
        }
        continue;
      }
    /* ---------- push into IFQ ---------- */
  ...
  if (stall && in_timing) fetch_ifq_full++;
...
static void rename_stage()  {
  ...
  while (renamed < sim_outorder_width) {
    ...
    int src1_phy = (a1 == 31) ? -1 : tctx[tid].rename_map[a1];
    int src2_phy = (a2 == 31) ? -1 : tctx[tid].rename_map[a2];
    
    enum md_opcode op;
    MD_SET_OPCODE(op, fe.inst);
    int new_phy = -1, old_phy = -1;
    if (alpha_writes_dest(fe.inst)){
      ...
      if (arch != 31) {
        ...
        old_phy = tctx[tid].rename_map[arch];
        tctx[tid].rename_map[arch] = new_phy;
        prf_ready[new_phy] = 0;
      }
    }
    ...
    renamed++;
    tctx[tid].rob_tail = (rob_tail_global + ROB_SIZE - 1) % ROB_SIZE;
  }
  if (rob_stall) rename_rob_full++;
  if (lsq_stall) rename_lsq_full++;
}
...
static void commit_stage(void)
{
    ...
    while (commits < sim_outorder_width) {
      ...
      tctx[re->tid].rob_head = (rob_head_global + 1) % ROB_SIZE;
      memset(re, 0, sizeof(*re));
      if (re->is_store) {
        lsq_head = (lsq_head + 1) % LSQ_SIZE;
      }
